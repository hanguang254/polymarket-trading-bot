...
- **Fee-Aware Entry EV**: Entry EV deducts only the official taker entry fee from Polymarket's fee formula. Bid/ask spread is tracked as a liquidity reference and handled by exit logic using live bids, not pre-deducted from entry EV.
...
- **Adaptive Warmup Sampling**: `WARMUP_SAMPLE_INTERVAL_EARLY` intervals before early window, accelerates to `WARMUP_SAMPLE_INTERVAL_LATE` after early window opens. Warmup starts at `WARMUP_START_SECONDS`. Default sampling source is `WARMUP_SAMPLING_SOURCE=BINANCE_SPREAD`, which feeds Bayesian warmup with Binance price plus the recent Chainlink-Binance offset, then falls back to Chainlink/BN if unavailable.
...
- **Binance Spread Sampling**: `sample_price - PTB` equals the offset-adjusted Binance lead. Logs show `(BN_SPREAD)`, raw Binance, and offset for traceability.
