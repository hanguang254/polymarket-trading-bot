...
- **Hyperbolic Discounting Profit-Take (P0) + Trailing TP (v12.8)**: 两层止盈系统。层1: 动态阈值 `base × (1 + k × minutes_remaining)`，利润达标时激活层2。层2: Trailing Take-Profit — 追踪持仓期间最高利润(high_water_mark)，从峰值回撤超过动态阈值就止盈。回撤阈值按利润等级：>30%利润容忍30%回撤, >20%容忍35%, >15%容忍40%。剩余时间<120s时回撤容忍度自动收紧。方向正确+ATR≥2.0时放宽20%给更多空间。解决了"峰值+22.8%一路滑到+4.5%才止盈"的核心问题。Configurable via `P0_BASE_PROFIT` (default 0.15), `P0_HYPERBOLIC_K` (default 0.15), `TRAILING_TP_ENABLED` (default 1).
...
- **价格源观测日志**: 每轮监控打印 `📡 源观测` 行，展示 CL/Pyth/BN 三路 age + staleness + 跨源 skew（ATR 单位）+ 盘口 spread + 唤醒类型。`get_current_crypto_price_debug()` 返回完整选源路径和 age_ms。
- **Oracle Stale Watch**: `evaluate_oracle_stale_watch()` 观测 Chainlink(结算源) 是否落后于 Binance(快市场)。剩余 ≤60s 时，|CL-BN| ≥ 0.5 ATR 连续 3 轮确认触发告警（`⚠️ 快市场领先告警`），偏离消失时打印恢复。**仅告警不干预交易决策**，用于事后分析结算价滞后导致的止损/方向判定偏差。可通过 `ENABLE_ORACLE_STALE_WATCH=0` 关闭。
...
- **v12.8.0**: WS 事件驱动 + Trailing Take-Profit + 狙击线程全面优化 — **WS 事件驱动**：狙击线程从固定 `sleep(50ms)` 改为 `poly_ws.wait_for_update()` 事件驱动，WS 推送到达即刻扫描（age ~0ms）。Polymarket WS 订阅修复：初始连接用 `type: market + initial_dump: true`，动态追加用 `operation: subscribe`（修复追加订阅无数据推送的根因）。`_on_message` 内所有 BBA 更新路径触发 `_price_event.set()`。**价格获取优化**：`get_realtime_odds()` / `get_market_price()` / `get_best_bid()` / `get_best_ask()` / `_get_orderbook_bids_asks()` 全部加入 WS 数据新鲜度快速路径——WS 有新鲜数据时跳过 REST 调用（省 ~300ms/次）。狙击线程 REST 校正改为按需触发（WS age < 阈值时跳过）。持仓脚本 `seed_from_rest` 确保首轮即有 WS 数据。**Trailing Take-Profit**：利润首次达到 P0 阈值后激活追踪止盈，记录持仓期间最高利润(HWM)。从峰值回撤超过动态阈值止盈——回撤阈值按利润等级分三档（>30%/20%/15% → 容忍30%/35%/40%回撤），剩余时间<120s自动收紧。解决"BTC +22.8%峰值滑到+4.5%才走"的核心问题。**狙击线程修复**：修复 `gap_dir` 未定义导致 NameError 静默崩溃；狙击线程主动获取 token（不等预热）；WS 自动订阅用 token_id 而非 slug 判断（跨 epoch token 不同）。**狙击时间窗口扩大**：从固定 `5s-160s` 改为"PTB就绪→结束前5s"，覆盖更长的观察期。**持仓脚本优化**：主循环唤醒改为 CLOB WS + Chainlink 双源事件驱动（50ms 兜底）；订阅后 `seed_from_rest` 确保首轮有数据；WS 诊断日志改用 logger（进 bot 日志）。**env 模板**：`.env.example` 全量注释补全（11 个模块分区 + 数值示例 + 时间线图）。

- **v12.7.3**: 主线程动量狙击 WS→REST 价格校正 — 修复主线程动量狙击使用 WS 虚假低价（如 $0.53）绕过 `SNIPER_MAX_PRICE` 检查、实际以 REST 真实高价（$0.73）成交的问题。狙击线程已有此校正（v12.6），现主线程对齐。Telegram 通知三路区分：🔫狙击线程(独立) / ⚡动量狙击(主线程) / 🎯正常下注。`.env.example` 新增狙击线程全量配置参数文档。
