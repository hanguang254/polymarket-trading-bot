# Polymarket Trading Bot v12.9.12
...
- **Directional Ambush Orders (v12.9.1)**: GTD limit order strategy — when sniper detects confirmed direction (Bayesian + gap aligned) but ask exceeds `SNIPER_MAX_PRICE` or WS is stale, places a GTD BUY limit on the confirmed direction only. Reprice attempts now keep tracking every previous order ID until expiry/cancel resolution, so failed cancel responses no longer create unmanaged ghost orders. Early transport-uncertain FOK attempts are also persisted into `pending_orders`, and reconciliation can merge addon fills back into an existing open position instead of silently resolving them away. Post-fill direction validation still re-checks Bayesian direction and confidence — if direction flipped or confidence < `SNIPER_AMBUSH_FILL_MIN_CONF` (default 15%), immediately FOK sells instead of creating position. Anti-loop guards remain in place (no placement when remaining <= END+5s, 30s cooldown after cancellation). Kelly-sized positions (MIN/MAX_BET_SIZE bounds). Default off (`SNIPER_AMBUSH=0`).
