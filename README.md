# Polymarket Trading Bot v12.9.3
...
- **Directional Ambush Orders (v12.9.1)**: GTC limit order strategy — when sniper detects confirmed direction (Bayesian + gap aligned) but ask exceeds `SNIPER_MAX_PRICE` or WS is stale, places a GTC BUY limit at `SNIPER_AMBUSH_PRICE` on the confirmed direction only. Checks token balance every 2s for fill detection, writes to `positions.jsonl` on fill. Cancel-before-check ordering prevents losing fill records. Anti-loop guards: no placement when remaining <= END+5s, 30s cooldown after cancellation. Kelly-sized positions (MIN/MAX_BET_SIZE bounds). Market cleanup auto-cancels residual GTC orders. Default off (`SNIPER_AMBUSH=0`).
...
| `SNIPER_AMBUSH_START` | No | Ambush window start, remaining < this seconds (default: 240) |
